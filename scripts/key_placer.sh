...
for file_path_map in "${files[@]}"; do
  ...
  gcloud kms $1 --ciphertext-file=$encrypted_file_path --plaintext-file=$file_path --key=staked-celo-keys --keyring=staked-celo-rings --location=global --project $environment
  if [[ $? -eq 1 ]]; then
